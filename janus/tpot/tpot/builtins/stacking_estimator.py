...
class StackingEstimator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # TODO: note that if prediction is string, will make entire matrix
        # string, so instead we try to get the label index
        preds = self.estimator.predict(X)
        if not np.issubdtype(preds.dtype, np.number):
            classes_ = self.estimator.classes_.tolist()
            preds = [classes_.index(p) for p in preds]
        X_transformed = np.hstack((np.reshape(preds, (-1, 1)), X_transformed))
