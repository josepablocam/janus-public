...
class TPOTBase(BaseEstimator):
    ...
    def __init__(self, generations=100, population_size=100, offspring_size=None,
                 ...
                 id_tracker=0,
                 ranking_model=None,
                 include_imputer_in_config=False,
                 ):
        ...
        self.failed_individuals_ = None
        self.include_imputer_in_config = include_imputer_in_config
    ...
    def _setup_config(self, config_dict):
        ...
        if not self.include_imputer_in_config:
            # remove imputer from dictionary, since don't need it
            if 'sklearn.impute.SimpleImputer' in self._config_dict:
                self._config_dict.pop('sklearn.impute.SimpleImputer')
    ...
    def record_pipeline_exception(self, exception, pipeline_str, pipeline_obj):
        self.failed_individuals_[pipeline_str] = {
            "exception": exception,
    ...
    def _check_dataset(self, features, target, sample_weight=None):
        ...
        if sparse.issparse(features):
            ...
            # JANUS: don't impute, instead we added impute to
            # configuration
            if not self.include_imputer_in_config and self._imputed:
                features = self._impute_values(features)

        if self.include_imputer_in_config:
            if target is None:
                return features
            else:
    ...
    def _combine_individual_stats(self, operator_count, cv_score, individual_stats, fit_time=None, pipeline_obj=None, individual_id=None, timestamp=None):
        ...
        if pipeline_obj is not None:
            stats['pipeline_obj'] = deepcopy(pipeline_obj)
        if individual_id is not None:
            stats['individual_id'] = individual_id
        if timestamp is not None:
            stats['timestamp'] = timestamp
        return stats

    def _rank_individuals_with_model(self, sklearn_pipelines, entries):
        if self.ranking_model is None:
            return entries
        # TODO: actually do this
        # predict time and score
        start_pred_time = datetime.now()
        scores = self.ranking_model.predict(sklearn_pipelines)
        # pareto frontier
        keep_ix = self.ranking_model.pareto_frontier_indices(scores)
        new_entries = [
            [_list[ix] for ix in keep_ix]
            for _list in entries
        ]
        end_pred_time = datetime.now()
        pred_time = end_pred_time - start_pred_time
        # adjust time tracker for this
        self.init_timestamp += pred_time
        return new_entries
    ...
    def _evaluate_individuals(self, population, features, target, sample_weight=None, groups=None):
        ...
        if self.ranking_model is not None:
            print("Using ranking model")
            (operator_counts, eval_individuals_str, sklearn_pipeline_list, stats_dicts) = self._rank_individuals_with_model(
                sklearn_pipeline_list, [operator_counts, eval_individuals_str, sklearn_pipeline_list, stats_dicts]
            )
        ...
        try:
            ...
            if self._n_jobs == 1 and not self.use_dask:
                for ix, sklearn_pipeline in enumerate(sklearn_pipeline_list):
                    self._stop_by_max_time_mins()
                    val, fit_time = partial_wrapped_cross_val_score(sklearn_pipeline=sklearn_pipeline)
                    if isinstance(val, BaseException):
                        self.record_pipeline_exception(
                            exception=val,
                            pipeline_str=eval_individuals_str[ix],
                            pipeline_obj=sklearn_pipeline,
                        )
                        # set to original value, so rest of tpot doesn't break...
                        val = -float('inf')
                    result_score_list = self._update_val(val, result_score_list)
                    result_fit_time_list.append(fit_time)
        ...
        self._update_evaluated_individuals_(result_score_list, eval_individuals_str, operator_counts, stats_dicts, sklearn_pipeline_list, result_fit_time_list, timestamp_list)
    ...
    def _update_evaluated_individuals_(self, result_score_list, eval_individuals_str, operator_counts, stats_dicts, sklearn_pipeline_list=None, result_fit_time_list=None, timestamp_list=None):
        ...
        None
        """
        for ix, (result_score, individual_str) in enumerate(zip(result_score_list, eval_individuals_str)):
            if type(result_score) in [float, np.float64, np.float32]:
                fit_time = result_fit_time_list[ix] if result_fit_time_list is not None else None
                pipeline_obj = sklearn_pipeline_list[ix] if sklearn_pipeline_list is not None else None
                individual_id = self.id_tracker
                timestamp = timestamp_list[ix] if timestamp_list is not None else None
